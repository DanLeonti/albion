...
export default function TopItemsRow({ item, rank }: TopItemsRowProps) {
  return (
    <tr className="border-b border-gray-800 hover:bg-gray-800/50 transition">
      ...
      <td className="py-2 px-3 text-right">
        <SilverDisplay amount={item.bestPrice} />
      </td>
      <td className="py-2 px-3 text-right text-sm text-gray-300">
        {item.bestCity}
      </td>
      <td
        className={`py-2 px-3 text-right text-xs ${getAgeColor(item.lastUpdated)}`}
      >
